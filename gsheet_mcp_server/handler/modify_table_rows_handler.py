"""
Handler for inserting and deleting rows within native Google Sheets tables using InsertDimensionRequest/DeleteDimensionRequest.

This module provides functionality to insert or delete rows within existing table objects.
"""
...
class TableRowsRequest(BaseModel):
    """Request model for inserting/deleting rows within native Google Sheets tables."""
...
def modify_table_rows(
    ...
    Raises:
        RuntimeError: If row operation fails
    """
    try:
        # Get spreadsheet ID
        ...
        return {
            ...
            "message": f"Successfully {operation_text} {count} rows at position {position} in table '{table_name}'"
        }
        ...
        raise RuntimeError(f"Error modifying table rows: {error_message}")
    except Exception as error:
        raise RuntimeError(f"Unexpected error modifying table rows: {error}")
